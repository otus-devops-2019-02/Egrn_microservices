...
jq '.|group_by(.g1)|map({ g1: .[0].g1, hosts: map(.ip), vars:{"ansible_python_interpreter":"/usr/bin/python3"} })|map({(.g1):{hosts,vars}})'`

groupArray2=`echo $rowList |
jq '.|group_by(.g2)|map({ g2: .[0].g2, hosts: map(.ip), vars:{"ansible_python_interpreter":"/usr/bin/python3"} })|map({(.g2):{hosts,vars}})'`

namesArray=`echo $rowList |
jq '.|group_by(.name)|map({ n: .[0].name, hosts: map(.ip), vars:{"ansible_python_interpreter":"/usr/bin/python3"}})|map({(.n):{hosts,vars}})'`


jq \
--argjson arr1 "$hostArray" \
--argjson arr2 "$allArray" \
--argjson arr3 "$groupArray1" \
--argjson arr4 "$groupArray2" \
--argjson arr5 "$namesArray" \
-n '$arr1 + $arr2 + $arr3 + $arr4 + $arr5 | map(.) | add' |
tee inventory.json
...
docker_machine_output() {
	rowOutputProd=`docker-machine ls | sed '1d' | awk -F' ' '{print $5}' | sed -E 's#[^/]+//([^:]+):.*#\1#'`
	for i in `echo "$rowOutputProd" | jq '.[]|.value[]' -cr`;
    do
        ssh-keygen -f "/root/.ssh/known_hosts" -R $i
        #until nc -vzw 2 $ipExt 22; do sleep 1; done
...
while true; do
  case "$1" in
    -l | --list ) make_json_list; docker_machine_output; exit 0 ;;
