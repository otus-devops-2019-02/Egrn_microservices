...
#owList=`gcloud compute instances list --format=json | jq '[.[]|{g1:.name|split("-")|.[0],g2:.name|split("-")|.[1],name,ip:.networkInterfaces[].accessConfigs[].natIP}]'`
rowList=`gcloud compute instances list --format=json | jq '[.[]|{g1:.name|split("-")|.[0],g2:.name|split("-")|.[1],name,ip:.networkInterfaces[].accessConfigs[].natIP}|select(.ip|length>0)]'`
...
terraform_output() {
	rowOutputProd=`terraform output -json`
